...
def get_df_from_data(data_set="training"):
    ...
    :param data_set: string, type for dataset ["sample", "training", "development", "test"]
    :return: file name and dataframe w.r.t. dataset
    """

    # to handle different naming convention in test set
    if data_set == "test":
...
def main():
    data_set = ["sample", "training", "development", "test"]
    for i in data_set:
        for j in get_df_from_data(i):
            print(j)
            print()
